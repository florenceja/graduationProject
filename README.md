...
- 转换脚本支持流式扫描 OAG zip；
- `edane_full_pipeline.py` 的 file 模式已改为 **流式扫描 + 快照分桶（磁盘临时文件）**，不再整读 CSV；
- 但即使读数不再是瓶颈，真正的“全量训练/评估”仍然受 **邻接矩阵 + 嵌入矩阵** 内存约束影响，通常仍需要结合 `--max-nodes`，或在高内存服务器上运行。
- 全量建议显式指定 `--partition-dir` 到 NVMe 路径，避免系统临时盘空间不足。
...
- 本项目当前 file 模式的**加载阶段**已改为 **流式扫描 + 快照分桶写盘（磁盘临时文件）**，不再要求整读 `edges.csv/features.csv/labels.csv`；
  但“全量转换成功”仍不等于“可直接全量训练/评估”，因为训练/更新阶段仍受 **CSR 邻接矩阵 + 多份 embedding/attrs 的常驻内存** 限制，通常需要结合 `--max-nodes` 渐进放量。
...
2. **（已完成）file 模式流式/采样式加载**（推荐默认）
   - 通过 `--node-selection-mode bounded` + 两遍扫描 + 分桶写盘，实现“先选节点 → 再分桶 → 再逐快照训练”
   - 作用是降低加载阶段的内存峰值，但不保证训练阶段能承载“真全量”
...
| [全量跑通指南](docs/full_scale_runbook.md) | 全量数据跑通的推荐命令、参数与排查 |
| [按RAM给出跑通方案](docs/oag_full_run_by_ram.md) | 不同内存条件下的可行路线与配置建议 |
