...
> 重要：本项目的 file 模式已改为 **流式扫描 + 快照分桶（磁盘临时文件）**，因此“全量 OAG 可行”仍分为两件事：
> 1) 能否完成全量转换；
> 2) 转换后能否直接全量训练/评估。
> 当前代码更倾向于 **(1) 可在大机器上完成**；同时流式改造能显著降低“读数阶段”的内存压力。
> 但 **(2) 是否能直接全量训练/评估** 仍取决于你是否能承载 CSR 邻接矩阵与 embedding 的内存/时间开销。
...
- 旧版 `build_graph_from_files()` 会先将 CSV 整体读入内存后再裁剪；当前版本已改为“先选节点 → 再分桶写盘 → 再逐快照训练”
...
方案 B（已完成/推荐）：**流式/采样式加载（file 模式）**

- 目标：让 file 模式在 `--max-nodes` 前就能预筛选
- 现已通过 `--node-selection-mode bounded` + 二次扫描分桶实现：避免“全量 CSV 先进入内存”
