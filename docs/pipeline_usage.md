...
注意：全量转换 ≠ 当前主流水线就能直接全量训练。
当前 file 模式的**加载阶段**已改为“流式扫描 + 快照分桶写盘”，不再要求整读 CSV；但训练/更新阶段仍受 CSR 邻接矩阵与多份 embedding/attrs 常驻内存限制，通常仍需要结合 `--max-nodes` 渐进放量或做子图采样。
...
| `--time-quantile-sample` | `1000000` | int | file 模式按 time 分位点切分快照时的采样容量（reservoir sampling） | file |
| `--partition-dir` | `""` | 字符串路径 | file 模式快照分桶临时目录（建议 NVMe；为空则用系统临时目录） | file |
| `--keep-partitions` | `False` | flag | 保留分桶文件（用于排查/复跑；默认会清理临时目录） | file |
| `--node-selection-mode` | `auto` | `auto`/`exact`/`bounded` | max-nodes 下节点筛选的度数统计方式（bounded 为近似 Top-K，节省内存） | file |
| `--node-selection-capacity` | `0` | int | bounded 模式计数器容量（默认约 max-nodes*5） | file |
| `--stable-batches` | `False` | flag | 对每个快照的边变化排序，保证完全确定性（全量大图会显著变慢） | file |
| `--no-all-results` | `False` | flag | 不更新项目根目录 `all_results.csv`（推荐全量长跑时开启，避免污染仓库） | 全部 |
