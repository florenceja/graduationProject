...
| 数据检查脚本 | `src/prepare_datasets.py` | 检查 `dataset/OAG/` 是否满足输入要求 |
...
如果 `dataset/OAG/` 下还是原始 `v5_oag_publication_*.zip`，先转换：
...
- file 模式不再按 `data/<preset>/` 查找输入。
- 后续真实实验统一按 `dataset/OAG/` 组织与运行。
