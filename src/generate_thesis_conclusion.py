...
def _dataset_sentence(row: Dict[str, str]) -> str:
    ...
    return (
        ...
        f"Micro-F1={micro_f1:.4f}、AUC={auc:.4f}，平均更新时延={avg_upd:.2f} ms。"
    )
...
def generate_markdown(rows: List[Dict[str, str]]) -> str:
    ...
    lines.append(
        f"链路预测 AUC 覆盖区间为 **{min(aucs):.4f}~{max(aucs):.4f}**，均高于随机水平；"
    ...
    lines.append("")
    lines.append(
        "综合实验结果可见，EDANE 在多数据集上均能稳定完成动态图嵌入学习，并在链路预测任务上取得稳定优势，"
    ...
    lines.append(
        ...
        f"平均 AUC={mean(aucs):.4f}，平均 Micro-F1={mean(micros):.4f}。"
    )
    lines.append("")
    return "\n".join(lines)
