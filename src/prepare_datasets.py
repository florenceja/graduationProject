...
def convert_oag_archives(
    ...
    # dry_run 只做扫描统计，不会写最终 CSV；因此允许在目标文件已存在时运行，
    # 避免用户为拿统计量而被迫覆盖本地现有数据。
    ...
    checkpoint_path = os.path.join(output_dir, "oag_convert_checkpoint.json")
    db_path = os.path.join(output_dir, "oag_meta_resume.sqlite") if resume else ""
    should_cleanup_db = not resume
    if resume:
        if overwrite:
    ...
    def _save_checkpoint(extra: Optional[Dict[str, object]] = None) -> None:
        if not resume:
            return
        payload = dict(checkpoint_state)
        if extra:
    ...
    try:
        ...
        conn.execute(
            "CREATE TABLE IF NOT EXISTS papers (paper_id TEXT PRIMARY KEY, year INTEGER NOT NULL, venue TEXT NOT NULL, kept INTEGER NOT NULL DEFAULT 0, label INTEGER NOT NULL DEFAULT -1, outgoing_count INTEGER NOT NULL DEFAULT 0, incoming_count INTEGER NOT NULL DEFAULT 0, score REAL NOT NULL DEFAULT 0.0)"
        )
        conn.execute("CREATE TABLE IF NOT EXISTS ref_counts (paper_id TEXT PRIMARY KEY, cnt INTEGER NOT NULL DEFAULT 0)")

        insert_sql = "INSERT OR IGNORE INTO papers(paper_id, year, venue, outgoing_count) VALUES(?, ?, ?, ?)"
        # 兼容旧版 SQLite：避免使用 INSERT ... ON CONFLICT DO UPDATE 语法
        ref_upsert_insert_sql = "INSERT OR IGNORE INTO ref_counts(paper_id, cnt) VALUES(?, 0)"
        ref_upsert_update_sql = "UPDATE ref_counts SET cnt = cnt + 1 WHERE paper_id = ?"
        batch: List[Tuple[str, int, str, int, List[str]]] = []
        inserted_papers = 0
        selection_strategy = selection_strategy.strip().lower()
        candidate_limit = 0

        resume_records_total = 0
        if resume and not overwrite and os.path.exists(checkpoint_path):
            try:
                with open(checkpoint_path, "r", encoding="utf-8") as f:
                    ckpt = json.load(f)
                if str(ckpt.get("phase", "pass1")) == "pass1":
                    resume_records_total = int(ckpt.get("records_total", 0))
                    print(f"[OAG resume] 从 records_total={resume_records_total} 继续 pass1 扫描")
            except Exception:
                resume_records_total = 0
        if max_papers > 0:
        ...
        for obj, _, _ in _iter_oag_records(
            ...
            if resume_records_total > 0 and stats["records_total"] <= resume_records_total:
                continue
            ...
            if should_flush:
                ...
                if ref_batch:
                    conn.executemany(ref_upsert_insert_sql, ref_batch)
                    conn.executemany(ref_upsert_update_sql, ref_batch)
                conn.commit()
            ...
            if report_every > 0 and stats["records_total"] % report_every == 0:
                ...
                checkpoint_state["phase"] = "pass1"
                checkpoint_state["records_total"] = int(stats["records_total"])
                if checkpoint_every > 0 and stats["records_total"] % checkpoint_every == 0:
                    _save_checkpoint()
        ...
        checkpoint_state["phase"] = "post_pass1"
        checkpoint_state["records_total"] = int(stats["records_total"])
        _save_checkpoint()
        conn.execute(
        ...
        checkpoint_state["phase"] = "done"
        checkpoint_state["records_total"] = int(stats.get("records_total", 0))
        _save_checkpoint({"completed": True})
        return dict(stats)
    finally:
...
def main() -> None:
    ...
    parser.add_argument("--resume", action="store_true", help="启用断点续跑：中断后可从 pass1 最近检查点继续")
    parser.add_argument("--checkpoint-every", type=int, default=200000, help="断点检查点间隔（按已扫描记录数）")
